...
// Validate file size
if ($_FILES['upload']['size'] > $typset_settings->limits->upload_file_size):
	unlink($_FILES['upload']['tmp_name']);
	$typset->respond(array(
		"status" => "error",
		"message" => "File is too large. It must be under " . $typset_settings->limits->upload_file_size / 1000000 . "MB"
	));
endif;
...
if ($original_pixels > $typset_settings->limits->upload_image_resolution):
	unlink($image);
	$typset->respond(array(
		"status" => "error",
